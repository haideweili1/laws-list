...
- **条目已有有效链接时，直接复用它，不要另编**：本次清单在『正文链接=已有』的条目，说明已经有能打开的官方链接。若你只变更 status / effectiveDate / remark 等非链接字段，source_url 直接填该条目**现有的 link**（它是真实官方正文页，能通过校验），不要去编新的链接；也不要为了"显得有更新"去改 link 字段。
